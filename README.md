...
slice now accepts explicit `and who`/`and that` coordination and finite
single-marker coordinated forms where later markers are elided, so
...
coordinated `and who`/`and that` counterparts and finite single-marker
...
ordinary analysis, the no-port download artifact check validates status,
`Content-Type`, `Content-Length`, `Content-Disposition`, and parsed JSON payload
against the same draft response, and the page's raw draft JSON preview must
match the API payload exactly. The page also mirrors the accepted-example list,
registration test draft, patch-text preview, download filename, and their
derived counts through stable `data-rule-draft-*` hooks, so a stale review panel
cannot hide behind the raw JSON block. The same promotion contract also cross-checks the
...
now itself verified and the surface witness audit covers the modified
transitive Adv family, the manner/location intransitive Adv family, and the
mixed Source/Goal/Instrument/Location/Manner intransitive Adv family, the
...
`Transition` truth before those fields are repackaged as an
`AtomicValuationSpec`. The follow-up verified
...
`ConcreteRegisteredTruthConditionModel`. It proves that the model denotation
feeds the generated spec denotation and that both the model route and the
spec-denotation route imply `AtomicClosureTruth`. This gives the finite
registered truth-condition instance a named model boundary while still leaving
independently justified general truth conditions open. The follow-up verified
objective
...
still finite and generated from the exported examples. The next verified
objective `coq_finite_registered_atomic_source_discipline_certificate` adds
`FiniteRegisteredAtomicSourceDisciplineCertificate`, which records the
registered evidence source behind each finite atom. It exposes
`RegisteredLexicalApplicationTruth` sources for the generated lexical atoms,
...
projection. The next verified objective
`coq_finite_registered_atomic_truth_condition_evidence_ledger` adds
...
`AtomicClosureTruth`.
The next verified objective
`coq_concrete_truth_condition_provider_class_obligation_suite` adds
`ConcreteTruthConditionProviderClassObligationSuite`. This layer binds the
provider interface to the
`RegisteredTruthConditionConstructorClassProjectionObligationLedger`: it stores
the checked provider source, the constructor-obligation ledger source, direct
...
adds `ConcreteTruthConditionIndependentTemporalModelCandidateCertificate`.
This temporal model-candidate certificate anchors the temporal class-readiness
boundary to the provider temporal class certificate,
`IndependentRegisteredTemporalTruthConditionInstances`, and
`ConcreteRegisteredCompositionalModel`. For `at_T`, `during_T`, `before_T`,
`after_T`, `until_T`, and `since_T`, it stores provider truth and atomic
projections, constructor-ledger truth and atomic projections,
...
the page also avoids a second hard-coded surface-parser attribute table. It now
derives every surface witness row, slot-probe row, and matrix row from the same
manifest lists before accepting the page. A companion
...
`Mary laughed loudly in the park with a telescope yesterday` under
`temporal_manner_location_instrument_intransitive_predication`,
`Mary laughed loudly in the park near a window beside a shelf under a lamp with
...
`Mary laughed from a window with a camera yesterday` under
`temporal_source_instrument_directional_instrument_intransitive_predication`,
`Mary laughed into a room with a camera yesterday` under
...
`Mary laughed into a room with a camera beside a shelf loudly yesterday` under
...
`Mary laughed from a window with a camera beside a shelf loudly under a lamp
on a table yesterday` under
`temporal_source_instrument_location_manner_location_sequence_directional_instrument_location_manner_location_sequence_intransitive_predication`,
`Mary laughed into a room with a camera beside a shelf loudly under a lamp on
a table yesterday` under
...
`Mary laughed from a window into a room with a camera beside a shelf loudly
under a lamp on a table yesterday` under
...
`Mary laughed from a window with a camera beside a shelf loudly under a lamp
on a table near a door yesterday` under
...
`Mary laughed from a window with a camera beside a shelf loudly under a lamp
on a table with a microphone yesterday` under
...
`Mary laughed into a room with a camera beside a shelf loudly under a lamp on
a table with a microphone yesterday` under
...
`Mary laughed from a window into a room with a camera beside a shelf loudly
under a lamp on a table with a microphone yesterday` under
`temporal_source_goal_instrument_location_manner_location_sequence_instrument_tail_directional_instrument_location_manner_location_sequence_instrument_tail_intransitive_predication`, and
	`Mary laughed from a window with a camera beside a shelf loudly under a lamp
	on a table near a door with a microphone yesterday` under
	...
	`Mary laughed from a window with a camera beside a shelf loudly under a lamp
	on a table with a microphone near a door yesterday` under
	`temporal_source_instrument_location_manner_location_sequence_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_tail_intransitive_predication`,
	`Mary laughed into a room with a camera beside a shelf loudly under a lamp on
	a table with a microphone near a door yesterday` under
	`temporal_goal_instrument_location_manner_location_sequence_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_tail_intransitive_predication`,
	`Mary laughed from a window into a room with a camera beside a shelf loudly
	under a lamp on a table with a microphone near a door yesterday` under
	`temporal_source_goal_instrument_location_manner_location_sequence_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_tail_intransitive_predication`, and
		`Mary laughed from a window with a camera beside a shelf loudly under a lamp
		on a table near a door with a microphone near a window yesterday` under
		`temporal_extended_source_instrument_location_manner_location_sequence_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_tail_intransitive_predication`,
		`Mary laughed from a window with a camera beside a shelf loudly under a lamp
		on a table with a microphone near a door with a telescope yesterday` under
		`temporal_source_instrument_location_manner_location_sequence_instrument_location_instrument_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_tail_intransitive_predication`,
		`Mary laughed into a room with a camera beside a shelf loudly under a lamp on
		a table with a microphone near a door with a telescope yesterday` under
		`temporal_goal_instrument_location_manner_location_sequence_instrument_location_instrument_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_tail_intransitive_predication`,
		`Mary laughed from a window into a room with a camera beside a shelf loudly
		under a lamp on a table with a microphone near a door with a telescope
		yesterday` under
		`temporal_source_goal_instrument_location_manner_location_sequence_instrument_location_instrument_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_tail_intransitive_predication`, and
			`Mary laughed from a window with a camera beside a shelf loudly under a lamp
			on a table near a door with a microphone near a window with a telescope
			yesterday` under
			`temporal_extended_source_instrument_location_manner_location_sequence_instrument_location_instrument_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_tail_intransitive_predication`,
			`Mary laughed from a window with a camera beside a shelf loudly under a lamp
			on a table with a microphone near a door with a telescope near a window
			yesterday` under
			`temporal_source_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_intransitive_predication`,
			`Mary laughed into a room with a camera beside a shelf loudly under a lamp
			on a table with a microphone near a door with a telescope near a window
			yesterday` under
			`temporal_goal_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_intransitive_predication`,
			...
			telescope near a window yesterday` under
			`temporal_source_goal_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_intransitive_predication`, and
			`Mary laughed from a window with a camera beside a shelf loudly under a lamp
			on a table with a microphone near a door with a telescope near a window in
			the park yesterday` under
			`temporal_extended_source_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_directional_instrument_location_manner_location_sequence_instrument_location_instrument_location_tail_intransitive_predication`,
...
The manifest also exposes `surface_parser_coverage` for three audited front-end
...
`sleep/quietly` subject/predicate/modifier slot probes. The
`mixed_role_intransitive_adv_sequence` family audits the same unary type shape
over Source/Goal/Instrument/Location/Manner prefixes `1..7`, including
controlled subject, predicate, source, and timed full-sequence probes such as
`John walked from a doorway into a hallway with a telescope beside a shelf
loudly under a lamp on a table yesterday`. All three records keep
...
manner-two-location intransitive route
`/api/analyze?sentence=Mary+laughed+loudly+in+the+park+near+a+window&require_coq=1`,
requiring `manner_two_location_intransitive_predication_single_reading`, the
`explicit_agent_with_manner_and_two_location_adv` scope, and typed Adv
declarations for `loudly`, `in_park`, and `near_window` rather than entity
surrogates. Its timed variant
...
`explicit_agent_with_manner_and_two_location_adv_at_time` scope and no
construction-rule draft. It then requests the registered
manner-three-location intransitive route
`/api/analyze?sentence=Mary+laughed+loudly+in+the+park+near+a+window+beside+a+shelf&require_coq=1`,
requiring `manner_three_location_intransitive_predication_single_reading`, the
`explicit_agent_with_manner_and_three_location_adv` scope, and typed Adv
declarations for `loudly`, `in_park`, `near_window`, and `beside_shelf`
rather than entity surrogates. Its timed variant
`/api/analyze?sentence=Mary+laughed+loudly+in+the+park+near+a+window+beside+a+shelf+yesterday&require_coq=1`
must keep the same registered rule while rendering
...
It then requests the registered manner-location-instrument intransitive route
`/api/analyze?sentence=Mary+laughed+loudly+in+the+park+with+a+telescope&require_coq=1`,
...
It also requests the registered Instrument-only route
`/api/analyze?sentence=Mary+laughed+with+a+telescope+yesterday&require_coq=1`,
requiring `instrument_intransitive_predication_single_reading`, the
`explicit_agent_with_instrument_adv_at_time` scope, and a typed Adv declaration
for `with_telescope` rather than an Entity surrogate.
...
than Entity surrogates. It also requests the registered Source/Goal directional
route
`/api/analyze?sentence=Mary+laughed+from+a+window+yesterday&require_coq=1`,
requiring `directional_intransitive_predication_single_reading`, the
...
It also requests the registered Source/Goal+Instrument+Location+Manner+Location
sequence route
...
scope, typed Adv declarations through `on_table`, and no Entity surrogate for
any modifier in the sequence.
It also requests the registered
Source/Goal+Instrument+Location+Manner+Location-sequence+Instrument-tail route
`/api/analyze?sentence=Mary+laughed+from+a+window+with+a+camera+beside+a+shelf+loudly+under+a+lamp+on+a+table+with+a+microphone+yesterday&require_coq=1`,
requiring
...
the
	...
	scope, typed Adv declarations through `with_microphone`, and no Entity
	surrogate for any modifier in the sequence.
	It also requests the registered
	Source/Goal+Instrument+Location+Manner+Location-sequence+Instrument+Location-tail
	route
	`/api/analyze?sentence=Mary+laughed+from+a+window+with+a+camera+beside+a+shelf+loudly+under+a+lamp+on+a+table+with+a+microphone+near+a+door+yesterday&require_coq=1`,
		requiring
		...
		scope, typed Adv declarations through `near_door`, and no Entity surrogate for
		any modifier in the sequence.
		It also requests the registered
		Source/Goal+Instrument+Location+Manner+Location-sequence+Instrument+Location+Instrument-tail
		route
		`/api/analyze?sentence=Mary+laughed+from+a+window+with+a+camera+beside+a+shelf+loudly+under+a+lamp+on+a+table+with+a+microphone+near+a+door+with+a+telescope+yesterday&require_coq=1`,
		requiring
		...
		the
			...
			scope, typed Adv declarations through `with_telescope`, and no Entity
			surrogate for any modifier in the sequence.
			It also requests the registered
			Source/Goal+Instrument+Location+Manner+Location-sequence+Instrument+Location+Instrument+Location-tail
			route
			`/api/analyze?sentence=Mary+laughed+from+a+window+with+a+camera+beside+a+shelf+loudly+under+a+lamp+on+a+table+with+a+microphone+near+a+door+with+a+telescope+near+a+window+yesterday&require_coq=1`,
			requiring
...
payload for each response. The same patch-text artifact contract is replayed
without opening an HTTP port, so the review text cannot drift from the fixed
bundle even when local networking is unavailable.
...
`SemanticPreservation` boundary as an inductive structural proof relation and a
`ModelInterpretable` boundary as a separate inductive model-interface relation.
For each checked example, the exporter creates a statement such as
...
operators, negation, Transition, and Cause. The generated theorem
`semantic_preservation_model_interpretable` proves that any structurally
preserved term is `ModelInterpretable`, and each exported example receives a
checked `example_i_model_interpretable` theorem by applying that bridge to its
structural proof. `scripts/check_formalization.py` requires these named theorem-obligation
rows, records, well-formedness proofs, record-binding
proofs, structural preservation proofs, and model interpretability boundary
proofs to stay one-to-one with the exported examples. This is still not a proof
of full denotational soundness: the remaining Coq work is to prove that
`ModelInterpretable` itself is sound with respect to a semantic interpretation.
