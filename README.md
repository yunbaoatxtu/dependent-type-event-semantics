...
typed Source+Instrument+Location+Manner+Location+Location+Instrument+Location+Instrument mixed modifier scaffold
`at_T(yesterday, laugh(9)(from(window), with(camera), beside(shelf), loudly, under(lamp), on(table), with(microphone), near(door), with(telescope), mary))`, and the
