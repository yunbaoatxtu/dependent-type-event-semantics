...
The verified test suite covers four positive translation cases and two negative type-checking cases. The positive cases cover variable polyadicity with time, argument omission, event counting, and causal resultatives. The negative cases reject an application whose adverb count does not match its modifier vector and a Cause term whose effect is not a Transition. These tests turn the theoretical decomposition into a sequence of small, reproducible implementation stages.
