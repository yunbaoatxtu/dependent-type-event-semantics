...
Well-typed ASTs can now be exported to a shallow proof-assistant embedding. The Lean-style exporter renders a sigma witness as Exists fun x_theme : Food => (eat 0 John x_theme), while the Coq-style exporter renders the same structure as exists x_theme : Food, (eat 0 John x_theme). Non-binding constructors use a shared prefix notation, for example repeat 2 (knock 0 John) and Cause John (Transition vase _ broken). Export is deliberately refused for ill-typed ASTs, so the proof-assistant boundary is guarded by the same structural checks used by the translator.

The verified test suite covers four positive translation cases, two negative type-checking cases, and one negative export case. The positive cases cover variable polyadicity with time, argument omission, event counting, and causal resultatives. The negative cases reject an application whose adverb count does not match its modifier vector, a Cause term whose effect is not a Transition, and an attempted proof-assistant export of an ill-typed AST. These tests turn the theoretical decomposition into a sequence of small, reproducible implementation stages.
